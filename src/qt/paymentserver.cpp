...
const char* BIP71_MIMETYPE_PAYMENT = "application/monoeci-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/monoeci-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/monoeci-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the monoeci: URI contains a payment request, we are not able to detect the
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click monoeci: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start monoeci: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling monoeci: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in monoeci: URIs
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    Q_FOREACH(const PAIRTYPE(CScript, CAmount)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom monoeci addresses are not supported
