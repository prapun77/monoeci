...
UniValue masternodelist(const JSONRPCRequest& request)
{
                ...
                strMode != "rank" && strMode != "sentinel" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print Monoeci address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"
                "  protocol       - Print protocol of a masternode (can be additionally filtered, exact match)\n"
                "  pubkey         - Print the masternode (not collateral) public key\n"
                "  rank           - Print rank of a masternode based on current block\n"
                "  sentinel       - Print sentinel version of a masternode (can be additionally filtered, exact match)\n"
                "  status         - Print masternode status: PRE_ENABLED / ENABLED / EXPIRED / WATCHDOG_EXPIRED / SENTINEL_PING_EXPIRED /\n"
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    { "monoeci",               "masternode",             &masternode,             true,  {} },
    { "monoeci",               "masternodelist",         &masternodelist,         true,  {} },
    { "monoeci",               "masternodebroadcast",    &masternodebroadcast,    true,  {} },
    { "monoeci",               "getpoolinfo",            &getpoolinfo,            true,  {} },
    { "monoeci",               "sentinelping",           &sentinelping,           true,  {} },
#ifdef ENABLE_WALLET
    { "monoeci",               "privatesend",            &privatesend,            false, {} },
#endif // ENABLE_WALLET
};
