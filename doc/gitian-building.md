...
in the Monoeci Core repository under 'Fetch and create inputs' to install sources which require
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/monoeci/.git/
...
For example:
```bash
URL=https://github.com/crowning-/monoeci.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit monoeci=${COMMIT} --url monoeci=${URL} ../monoeci/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit monoeci=${COMMIT} --url monoeci=${URL} ../monoeci/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit monoeci=${COMMIT} --url monoeci=${URL} ../monoeci/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the monoeci git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../monoeci/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/monoeci
SIGPATH=/some/root/path/monoeci-detached-sigs

./bin/gbuild --url monoeci=${BTCPATH},signature=${SIGPATH} ../monoeci/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/monoeci-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/monoeci-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/monoeci-osx-build.assert
```
